...
namespace twlm::ccpl::modules
{
    ...
    /**
     * AST Builder
     * Provides helper functions to build AST nodes during parsing
     */
    class ASTBuilder
    {
    private:
    ...
    public:
        ...
        // Convert expression linked list to vector (for function arguments)
        std::vector<std::shared_ptr<Expression>> expr_list_to_vector(std::shared_ptr<Expression> expr_list);
        
        // Statement builders
        std::shared_ptr<Statement> make_expr_stmt(std::shared_ptr<Expression> expr);
        std::shared_ptr<BlockStmt> make_block();
        std::shared_ptr<BlockStmt> make_block_from_list(std::shared_ptr<Statement> stmt_list);
        ...
        // Helper: join statements into linked list
        std::shared_ptr<Statement> join_statements(std::shared_ptr<Statement> s1,
                                                    std::shared_ptr<Statement> s2);
        
        // Helper: convert param linked list to vector
        std::vector<std::shared_ptr<ParamDecl>> param_list_to_vector(std::shared_ptr<ParamDecl> param_list);
        
        // Helper: convert var decl linked list to vector
        std::vector<std::shared_ptr<VarDecl>> var_decl_list_to_vector(std::shared_ptr<VarDecl> decl_list);
